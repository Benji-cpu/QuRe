...
const CallForm: React.FC<CallFormProps> = ({ value, onChange }) => {
  ...
  useEffect(() => {
    if (value && value.startsWith('tel:')) {
      try {
        ...
        const telNumber = value.substring(4).replace(/[^0-9+]/g, '');
  ...
  // Update the tel: URL whenever inputs change
  useEffect(() => {
    if (validatePhone()) {
